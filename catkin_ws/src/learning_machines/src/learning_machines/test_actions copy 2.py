...
def get_reward(rob, start_pos, movement, speeds):
    ...
    reward += (distance * 100)  # Ensure this scaling is appropriate
...
def run_training_simulation(rob, agent, num_episodes, max_steps_per_episode=20):
...
def run_trained_model(rob, model_path, num_steps=20):
    # Load the trained model
    state_dim = 8
    model = SpeedPredictor(state_dim).to(device)
    model.load_state_dict(torch.load(model_path))
    model.eval()

    # Run the model to predict speeds
    rob.play_simulation()
    state = get_state(rob)
    ...
    for step in range(num_steps):
        ...
        rob.move_blocking(left_speed, right_speed, 100)
        state = get_state(rob)
        print(f"Step: {step}, Left Speed: {left_speed}, Right Speed: {right_speed}")
...
def run_all_actions(rob):
    if mode == 'train':
        run_training_simulation(rob, agent, num_episodes=20)
    else:
        run_trained_model(rob, model_path)
